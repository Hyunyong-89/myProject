...
public abstract class DateUtils implements DateConstants {
     ...
     * 월 차이 계산
     *
     * @param 시작년도, 시작월, 종료년도, 종료일
     * @return 차이값
     */
    public static int diffrentMonth(String stYear, String stMonth, String endYear, String endMonth) {
        
    	int stY = Integer.parseInt(stYear);
    	int stM = Integer.parseInt(stMonth);
    	int endY = Integer.parseInt(endYear);
    	int endM = Integer.parseInt(endMonth);
    	
    	int result = ((endY*12)+endM) - ((stY*12)+stM); 
     ...
     * @return 여부
     */
    public static String addMonth(String year, String month, int addNum) {
        
    	String sdate = year + month+ "01";
    	String result = "";
		LocalDate localDate = LocalDate.parse(sdate, DateTimeFormatter.BASIC_ISO_DATE);
	    
		result = localDate.plusMonths(addNum).format(DateTimeFormatter.BASIC_ISO_DATE);
