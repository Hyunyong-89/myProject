...
public class ExcelController {
	...
	public void excelDownload(HttpServletResponse response) throws Exception {
		...
		List<String> dateList = taxBillMapper.selectTaxBillLGroupDate("20240508010001");
		int dateCnt = dateList.size();
		log.debug(dateList.toString());
		log.debug(Integer.toString(dateCnt));
		for(int i=0;i<dateCnt;i++) {
			...
			dataMap.put("prjCd", "20240508010001");
			dataMap.put("taxBillDate", dateList.get(i));
			List<TaxBillDto> list = taxBillMapper.selectTaxBillListByPrjCdTaxBillDate(dataMap);
			...
			for (int j=0; j<cnt; j++) {
				...
				log.debug(taxBillDto.writeDate);
			...
			String fileName = dateList.get(i)+"01_세금계산서등록양식(0000_"+year+"년"+month+"월분).xlsx";
			//log.debug(fileName);
